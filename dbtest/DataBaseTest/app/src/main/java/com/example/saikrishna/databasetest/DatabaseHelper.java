package com.example.saikrishna.databasetest;

/**
 * Created by sai krishna on 7/4/2017.
 */
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public List<Student> getEmployees() {
        ...
        String query = "SELECT _id,rollno FROM Employee";
        Cursor cursor = db.rawQuery(query, null);
        List<Student> list = new ArrayList<Student>();
        while(cursor.moveToNext()) {
            Student stu = new Student();
            stu.setId(cursor.getInt(0));
            stu.setRollno(cursor.getInt(1));
            list.add(stu);
        }
        db.close();
        return list;
